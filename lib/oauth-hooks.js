(function(exports) {
  ...
  exports.grantClientToken = function (credentials, req, cb) {
    ...
    if (!credentials.clientId || !credentials.clientSecret) {
      ...
      if (authHeader.scheme !== "Basic" || !authHeader.token || authHeader.token.length < 2) {
    ...
    var shareId = decodeURIComponent(credentials.clientId);
    log("grantClientToken for %s", shareId);
    authServer.adminApi.getAccountForUser(shareId)
      .then(function(share) {
        if (!share) {
          errLog("account not found: %s", shareId);
          return Promise.reject(new Error("no match found for " + shareId));
        }
        return checkAccountSecret(share, credentials.clientSecret);
      })
      .then(function(share) {
        if (!share) {
